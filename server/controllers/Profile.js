...
exports.instructorDashboard = async (req, res) => {
  try {
    ...
    //   .select("courseName price studentsEnrolled")
    //   .lean();
    const courses = await Course.find({ instructor: req.user.id })
  .populate("studentsEnrolled") // <- this is important
  .exec();

    console.log("Courses found for instructor:", courses);

    let totalStudents = 0;
    let totalIncome = 0;

    // Map over courses to calculate stats
    ...
    res.status(500).json({ message: "Could not get Instructor data" });
  }
};
